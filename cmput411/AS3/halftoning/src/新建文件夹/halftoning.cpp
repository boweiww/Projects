...
void hand_image(GLubyte * temp,float a, int er,int eg,int eb,int i, int j){
	temp[(i*imageSizeX + j) * 3] = limit_color (temp[(i*imageSizeX + j) * 3] + a * er);
	temp[(i*imageSizeX + j) * 3 + 1] = limit_color (temp[(i*imageSizeX + j) * 3 + 1] + a * eg);
	temp[(i*imageSizeX + j) * 3 + 2] = limit_color (temp[(i*imageSizeX + j) * 3 + 2] + a * eb);
...
#define DELTA 0.0625
	...
	for (i = 0; i < imageSizeY; i++) {
		for (j = 0; j < imageSizeX; j++)
		{
			...
			er = temp[(i*imageSizeX + j) * 3];		    //red
			eg = temp[(i*imageSizeX + j) * 3 + 1];		//green
			eb = temp[(i*imageSizeX + j) * 3 + 2];		//blue
