...
float cmin(float arr[4],int num){
    float min = arr[0];
...
void main(void )
{
    ...
    if (dist > MAX_DIST - EPSILON && dist2 > MAX_DIST - EPSILON && dist3 > MAX_DIST - EPSILON && dist4 > MAX_DIST - EPSILON  ) {
        // Didn't hit anything
    ...
    float dists [4] = float[4](dist,dist2,dist3,dist4);
    if (cmin(dists,4) == dist) {
        outColor = vec4(color1, 1.0);
    }
    else if (cmin(dists,4) == dist2) {
        outColor = vec4(color2, 1.0);

    }
    else if (cmin(dists,4) == dist3){
