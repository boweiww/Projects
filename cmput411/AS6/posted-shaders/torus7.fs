...
// float rotate(vec3 vector){
//     float y,z;
    
//     return (y,z);
// }
// why dose function not work
float cSDF(vec3 sample){
    float x,y,z;
    x = sample.x;
    y = sample.y*sqrt(8)/3 + sample.z/3;
    z = sample.z*sqrt(8)/3 - sample.y/3;    
    // y=y*1.025;
    // z=z*1.025;
    return sqrt(pow(sqrt (x*x + y*y)-0.3, 2) + z*z) - 0.05;
...
vec3 rotate(vec3 v){
    ...
    y = v.y*cos(iTime) + v.z*sin(iTime);
    z = v.z*cos(iTime) - v.y*sin(iTime);
    v.y = y;
    v.z = z;
    return v;
 ...
 */
 float SDFF(vec3 eye, vec3 marchingDirection, float start, float end) {
    ...
    for (int i = 0; i < MAX_MARCHING_STEPS; i++) {
        float dist = cSDF(rotate(eye + depth * marchingDirection));
        if (dist < EPSILON) {
			return depth;
        }
        depth += dist;
...
float shortestDistanceToSurface(vec3 eye, vec3 marchingDirection, float start, float end) {
    float depth = start;
    for (int i = 0; i < MAX_MARCHING_STEPS; i++) {
        float dist = sceneSDF(rotate(eye + depth * marchingDirection));
        if (dist < EPSILON) {
			return depth;
        }
        depth += dist;
...
void main(void )
{
    ...
    float dist4 = SDFF(eye, dir4, MIN_DIST, MAX_DIST);
    float dist5 = SDFF(eye, dir5, MIN_DIST, MAX_DIST);
    ...
    y = p4.y*sqrt(8)/3 + p4.z/3;
    z = p4.z*sqrt(8)/3 - p4.y/3;  
    p4.y = y;
    p4.z = z;
    vec3 p5 = eye + dist5 * dir5;
    y = p5.y*sqrt(8)/3 + p5.z/3;
    z = p5.z*sqrt(8)/3 - p5.y/3;  
    p5.y = y;
    p5.z = z;
    p = rotate(p);
    p2 = rotate(p2);
