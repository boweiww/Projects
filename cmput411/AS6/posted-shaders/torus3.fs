...
float cmin(float arr[3],int num){
    float min = arr[0];
...
void main(void )
{
    ...
    if (dist > MAX_DIST - EPSILON && dist2 > MAX_DIST - EPSILON && dist3 > MAX_DIST - EPSILON ) {
        // Didn't hit anything
    ...
    vec3 K_a3 = vec3(0.8, 0.8, 0.1);
    vec3 K_d3 = vec3(0.8, 0.8, 0.1);
    ...
    float dists [3] = float[3](dist,dist2,dist3);
    if (cmin(dists,3) == dist) {
        outColor = vec4(color1, 1.0);
    }
    else if (cmin(dists,3) == dist2) {
