...
void print_initial_state() {
    ...
    while ((entry = readdir(dr)) != NULL) {
        ...
        if(entry->d_type == DT_DIR) {
            ...
            new_dir.name = entry->d_name;

            // need to concatenate path with the new directory
            char filepath[FILEPATH_SIZE];
            strcpy(filepath, path);
            strcat(filepath, entry->d_name);

            new_dir.wd = inotify_add_watch(sub_fd, filepath, IN_CREATE | IN_MODIFY | IN_ATTRIB | IN_DELETE
...
void print_subdirectories() {
    ...
    int length = read_inotify(sub_fd, buffer);
...
void print_changes() {
    ...
    char buffer[EVENT_BUF_LEN];
    int length = read_inotify(main_fd, buffer);
    ...
    for (int i = 0; i < used_entries; ++i) {
        ...
        case CREATED:
            ...
            case S_IFDIR: printf("d %s\n",arr[i].name);
                          ...
                          struct watch new_dir;
                          new_dir.name = arr[i].name;

                          // first need to concatenate path with the new directory
                          char filepath[FILEPATH_SIZE];
                          strcpy(filepath, path);
                          strcat(filepath, arr[i].name);

                          new_dir.wd = inotify_add_watch(sub_fd, filepath,
                                   IN_CREATE | IN_MODIFY | IN_ATTRIB | IN_DELETE
                                 | IN_DELETE_SELF | IN_MOVE_SELF
                                 | IN_MOVED_FROM | IN_MOVED_TO);
