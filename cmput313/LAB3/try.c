...
void LANC_manager (int ePoint)
{
   ...
   switch(ePoint){
    ...
    case EP_app:
        assert ( (lanc.state == LC_idle) &&(lanc.f.dest != nodeinfo.address));
    ...
    case EP_collision:
        ...
        if(lanc.backoff >= MAX_BACKOFF){
            ...
            ++lanc.backoff;
            //int backoff_rn;
            int backoff_rn = lanc.backoff_rn;
            backoff_rn = CNET_rand();
            LANC_start_timer(backoff_rn * LAN_SLOTTIME,EP_backoff);
            break;
        }
    ...
    case EP_cs:
        if(CS_FLAG == 1){
            retVal = CNET_carrier_sense(lanc.link);
            if(retVal == 1){
                LANC_start_timer(LAN_TINYSLOT, EP_cs);
                break;
            }
